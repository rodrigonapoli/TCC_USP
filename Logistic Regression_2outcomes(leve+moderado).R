...
##   RESULTADO FINAL  ##
########################
# Neste modelo o psudo R2 foi de 71,76% (p<0,001). 
# Variáveis preditoras: SexoM, IMC, RCE, CP, tri, tgp, TGO_TGP 
# acc = 94,02%
# sensibilidade = 98,10%
# especificiadad = 58,33%
...
library ('dplyr') #para rodar alguns comandos como %>%
...
modelo.completo <- glm(esteatose_dic ~.,
...
predicted.classes <- ifelse(probabilities < 0.5, "1_2", "3")
#### Model accuracy
mean(predicted.classes==dados$esteatose_dic)
probabilities
predicted.classes

tab <- table(predicted.classes, dados$esteatose_dic)
tab
...
predito <- data.frame(
  ...
  esteatose_dic=dados$esteatose_dic)

predito <- predito[
  order(predito$probability.of.esteatose, decreasing = FALSE),]
predito$rank <- 1:nrow(predito)

ggplot(data=predito, aes(x=rank, y=probability.of.esteatose, color=esteatose_dic)) + 
  ggtitle("Modelo com 2 outcomes: Leve + Moderado VS Severo") +
  geom_point() +
  labs(x = "Index",
...
predicoes <- prediction(predictions = stepmodelo$fitted.values, 
                        labels = as.factor(dados$esteatose_dic))
...
ROC <- roc(response = dados$esteatose_dic, 
