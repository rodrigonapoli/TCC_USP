...
######## NAIVE BAYES
classificador <- naiveBayes(esteatose_dic2 ~ ., data = treino)
classificador
...
# Matriz de confusão
tab <- table(teste$esteatose_dic2, predito)
tab

confusionMatrix(tab)

#ROC
roc_NB_test <- roc(response = teste$esteatose_dic2, predictor =as.numeric(predito))
roc_NB_test
plot(roc_NB_test)
