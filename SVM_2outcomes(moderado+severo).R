...
n <- nrow(dados_norm)  #definindo o número total de observações
...
svm1 <- svm(esteatose_dic2~., data=traino_dados, 
...
tab <- table(teste_dados$esteatose_dic2, predict)
tab

#medindo a qualidade do modelo
acc <- (tab[1,1]+tab[2,2])/sum(tab) # Calculando a acurácia
acc
confusionMatrix(table(predict ,teste_dados$esteatose_dic2))

#ROC
roc_svm_test <- roc(response = teste_dados$esteatose, predictor =as.numeric(predict))
roc_svm_test
plot(roc_svm_test)
