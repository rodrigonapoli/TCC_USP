...
write_xlsx(ai,"TREE_importancia_semNA_agrupado_1_2E3.xlsx", col_names = TRUE, format_headers = TRUE)
...
class2 = base::factor(ifelse(prob[,2] >= 0.5, "2_3", "1"))
head(class2)

# Matriz de confusão
tab <- table(class2, dados$esteatose_dic2)
...
aval <- data.frame(obs=dados$esteatose_dic2, 
                   pred=class2,
                   "2_3" = prob[,2],
                   "1" = 1-prob[,2])
...
arvore_poda <- rpart::rpart(esteatose_dic2 ~ .,
                            data=dados,
                            method='class',
                            xval=0,
...
class2 = base::factor(ifelse(prob[,2]>.5, "2_3", "1"))
