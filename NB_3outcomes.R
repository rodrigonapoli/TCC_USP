...
######## NAIVE BAYES
classificador <- naiveBayes(esteatose ~ ., data = treino)
classificador
...
# Matriz de confusão
tab <- table(teste$esteatose, predito)
tab

confusionMatrix(tab)

#ROC
roc_NB_test <- multiclass.roc(response = teste$esteatose, predictor =as.numeric(predito))
roc_NB_test
plot(roc_NB_test)
